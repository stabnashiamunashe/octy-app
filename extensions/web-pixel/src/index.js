...
register(async ({analytics, browser, settings, init} ) => {
  ...
  let has_visited = await browser.cookie.get("has_visited") === "true" ? true : false;

  async function getClientData() {
    const response = await fetch('https://pod1.sonity.net/octy-client-info/info');
  ...
  if (!has_visited) {
    ...
    await browser.cookie.set(`fingerprint_id=${fingerprint_id};` + expire + ";path=/");
    ...
    try {
      ...
      const old_octy_customer_id = await browser.localStorage.getItem("octy_customer_id")
      ...
      const new_octy_customer_id = await octy_functions.createUpdateProfile(customer_id, fingerprint_id , shopify_customer_id , settings.accountID ) 
      await browser.localStorage.setItem("octy_customer_id", new_octy_customer_id)
  ...
  analytics.subscribe('checkout_started', async (event) => {
    ...
    const raw_octy_customer_id = await browser.localStorage.getItem('octy_customer_id');
